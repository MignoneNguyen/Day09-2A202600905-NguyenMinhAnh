    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves privacy, data, GDPR, personal information'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
def route_to_specialists(state: LegalState) -> list[Send]:
    ...
    if state.get("needs_tax") or any(kw in question_lower for kw in ["tax", "irs", "thuế"]):
        tasks.append(Send("call_tax_specialist", state))
    
    if state.get("needs_compliance") or any(kw in question_lower for kw in ["compliance", "sec", "regulation"]):
        tasks.append(Send("call_compliance_specialist", state))
    
    if state.get("needs_privacy") or any(kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu"]):
